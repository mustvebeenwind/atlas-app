...
function playYeatBell(ctx, { gain = 0.9, pitch = 880 } = {}) {
  ...
  const delayL = ctx.createDelay(); const delayR = ctx.createDelay(); delayL.delayTime.value = 0.008; delayR.delayTime.value = 0.011;
  const splitter = ctx.createChannelSplitter(2); const merger = ctx.createChannelMerger(2);
  env.connect(splitter); splitter.connect(delayL, 0); splitter.connect(delayR, 1); delayL.connect(merger, 0, 0); delayR.connect(merger, 0, 1); merger.connect(master);
  carrier.connect(env); partial.connect(env);
...
function ensureAudioContext(ref) {
  if (!ref.current) { const AC = window.AudioContext || window.webkitAudioContext; ref.current = new AC(); }
...
const clone = (v) => {
  ...
  try { return JSON.parse(JSON.stringify(v, (_k, val) => (typeof val === "function" || val === undefined ? null : val))); }
  catch { if (Array.isArray(v)) return v.map((x) => (x && typeof x === "object" ? { ...x } : x)); if (v && typeof v === "object") return { ...v }; return v; }
};
const num = (v, d = 0) => (Number.isFinite(Number(v)) ? Number(v) : d);
const round = (v, d = 0) => (Number.isFinite(Number(v)) ? Math.round(Number(v)) : d);
const fitWithin = (w, h, maxW, maxH) => { const W = num(w), H = num(h), MW = num(maxW), MH = num(maxH); if (W <= 0 || H <= 0 || MW <= 0 || MH <= 0) return { w: 0, h: 0 }; const r = Math.min(MW / W, MH / H); const out = { w: Math.round(W * r), h: Math.round(H * r) }; if (!Number.isFinite(out.w) || !Number.isFinite(out.h)) return { w: 0, h: 0 }; return out; };
const isSquareish = (w, h) => { const W = num(w), H = num(h); if (W <= 0 || H <= 0) return false; const r = W / H; return r > 0.9 && r < 1.1; };
const normRectLoose = (fx0, fy0, fx1, fy1) => { const L = Math.min(num(fx0), num(fx1)); const T = Math.min(num(fy0), num(fy1)); const R = Math.max(num(fx0), num(fx1)); const B = Math.max(num(fy0), num(fy1)); return { fx: L, fy: T, fw: R - L, fh: B - T }; };
...
const Icon = {
  ...
  chair: (s = 48) => (<svg width={s} height={s} viewBox="0 0 64 64" role="img" aria-label="chair"><rect x="22" y="14" width="20" height="14" rx="2" {...S} /><rect x="22" y="28" width="20" height="8" rx="2" /><line x1="24" y1="36" x2="24" y2="50" {...S} /><line x1="40" y1="36" x2="40" y2="50" {...S} /></svg>),
};
...
const styles = {
  ...
  resizeHandle: (color) => ({ position: "absolute", width: 8, height: 8, background: "#fff", border: `2px solid ${color}`, borderRadius: 0, boxShadow: "0 1px 2px rgba(0,0,0,0.15)", zIndex: 3, touchAction: "none" }),
...
export default function ImageCanvasApp() {
  ...
  const [bgSize, setBgSize] = useState({ baseW: DEFAULT_WORLD.baseW, baseH: DEFAULT_WORLD.baseH, naturalW: 0, naturalH: 0, scale: 1 });
  ...
  const [history, setHistory] = useState([{ items: [], walls: [], windows: [], floors: [], bgUrl: null, bgSize: { baseW: DEFAULT_WORLD.baseW, baseH: DEFAULT_WORLD.baseH, naturalW: 0, naturalH: 0, scale: 1 } }]);
  ...
  /* ========== FIXED: float-precise frame (no rounding) ========== */
  const getRefFrame = () => {
    ...
    const bgDx = (cw - dispW) / 2 + num(bgPan.x);   // floats!
    const bgDy = (ch - dispH) / 2 + num(bgPan.y);   // floats!
    return { left, top, cw, ch, baseW, baseH, scale, dispW, dispH, bgDx, bgDy, refW: baseW, refH: baseH };
  };

  const getRel = (x, y) => {
    const f = getRefFrame();
    return { fx: (x - (f.left + f.bgDx)) / (f.baseW * f.scale), fy: (y - (f.top + f.bgDy)) / (f.baseH * f.scale), refW: f.baseW, refH: f.baseH };
  };
  ...
  const onFile = (file) => {
    ...
    try {
      ...
      img.onload = () => {
        ...
        const baseW = base.w > 0 ? base.w : DEFAULT_WORLD.baseW; const baseH = base.h > 0 ? base.h : DEFAULT_WORLD.baseH;
  ...
  const setLayer = (k, updater) => { if (k === "wall") setWalls(updater); else if (k === "window") setWindows(updater); else setFloors(updater); };
  ...
  const handleCanvasClick = (e) => {
    ...
    snapshotLayer(activeTool, next); setDraft(null);
  ...
  const submitWindowHeight = (e) => {
    ...
    setWindows((prev) => { const next = prev.map((w) => (w.id === windowPrompt.id ? { ...w, heightCm: val } : w)); snapshotLayer("window", next); return next; });
    setWindowPrompt(null);
  };
  const onChangeWindowHeight = (e) => setWindowPrompt((p) => (p ? { ...p, value: e.target.value, error: "" } : p));
  const openWindowPrompt = (id, initialValue) => { const val = typeof initialValue === "number" && Number.isFinite(initialValue) ? String(initialValue) : ""; setWindowPrompt({ id, value: val, error: "" }); };
  ...
  const onCanvasPointerMove = (e) => {
    ...
    if (rs.id) {
      const { fx, fy } = getRel(e.clientX, e.clientY); const s = rs.start;
      let L = s.fx, T = s.fy, R = s.fx + s.fw, B = s.fy + s.fh; const ce = (v) => clamp01(num(v));
      ({ nw:()=>{L=ce(fx);T=ce(fy);}, ne:()=>{R=ce(fx);T=ce(fy);}, sw:()=>{L=ce(fx);B=ce(fy);}, se:()=>{R=ce(fx);B=ce(fy);}, n:()=>{T=ce(fy);}, s:()=>{B=ce(fy);}, w:()=>{L=ce(fx);}, e:()=>{R=ce(fx);} }[rs.handle] || (()=>{}))();
      L=Math.max(0,Math.min(L,1)); R=Math.max(0,Math.min(R,1)); T=Math.max(0,Math.min(T,1)); B=Math.max(0,Math.min(B,1));
    ...
    if (rsz.id) { const it = items.find((i)=>i.id===rsz.id); if (!it) return; const d=Math.max(e.clientX - rsz.startX, e.clientY - rsz.startY); const size=Math.max(16, Math.min(256, Math.round(num(rsz.startSize || 48)+d))); setItems((p)=>p.map((x)=>(x.id===rsz.id?merge(x,{size}):x))); return; }
    const sd = shapeDraggingRef.current;
    if (sd.id) { const { fx, fy } = getRel(e.clientX, e.clientY); const nx=(num(fx)-num(sd.offsetFx)); const ny=(num(fy)-num(sd.offsetFy)); setLayer(sd.kind,(p)=>p.map((r)=>(r.id===sd.id?merge(r,{fx:nx, fy:ny}):r))); return; }
    const drag = draggingRef.current;
    if (drag.id) { const { fx, fy } = getRel(e.clientX, e.clientY); setItems((p)=>p.map((it)=> it.id===drag.id ? merge(it,{ fx:(num(fx)-num(drag.offsetFx)), fy:(num(fy)-num(drag.offsetFy)) }) : it )); return; }
    ...
    if (e.pointerType === "touch" && pointersRef.current.has(e.pointerId)) {
      ...
      if (pointersRef.current.size === 2) {
        const [p1,p2]=Array.from(pointersRef.current.values()); const midX=(p1.x+p2.x)/2, midY=(p1.y+p2.y)/2;
        const dist=Math.hypot(p1.x-p2.x,p1.y-p2.y); if (!onCanvasPointerMove._lastDist) onCanvasPointerMove._lastDist=dist;
        const factor=dist/onCanvasPointerMove._lastDist; if (factor>0 && !Number.isNaN(factor)) { applyZoomAt(midX, midY, factor); const prev=onCanvasPointerMove._lastMid || {x:midX,y:midY}; setBgPan((p)=>({ x:p.x+(midX-prev.x), y:p.y+(midY-prev.y) })); onCanvasPointerMove._lastMid={x:midX,y:midY}; } onCanvasPointerMove._lastDist=dist;
      }
    }
  };
  ...
  const onCanvasPointerUp = (e) => {
    const rs = shapeResizingRef.current;
    if (rs.id) { const before = rs.startLayer; shapeResizingRef.current = { kind:null,id:null,handle:null,start:null,startLayer:null }; const after = getLayer(rs.kind); if (before && JSON.stringify(before)!==JSON.stringify(after)) snapshotLayer(rs.kind, after); }
    const rsz = resizingRef.current;
    if (rsz.id) { resizingRef.current = { id:null,startSize:0,startX:0,startY:0 }; const before = dragStartSnapshotRef.current || []; if (JSON.stringify(before)!==JSON.stringify(items)) snapshotItems(items); dragStartSnapshotRef.current = null; }
    const sd = shapeDraggingRef.current;
    if (sd.id) { const before = sd.start; shapeDraggingRef.current = { kind:null,id:null,offsetFx:0,offsetFy:0,fw:0,fh:0,start:null }; const after = getLayer(sd.kind); if (before && JSON.stringify(before)!==JSON.stringify(after)) snapshotLayer(sd.kind, after); }
    const drag = draggingRef.current;
    if (drag.id != null) { draggingRef.current = { id:null, offsetFx:0, offsetFy:0 }; const before = dragStartSnapshotRef.current || []; if (JSON.stringify(before)!==JSON.stringify(items)) snapshotItems(items); dragStartSnapshotRef.current=null; }
  ...
  const onCanvasPointerCancel = () => { pointersRef.current.clear(); panDragRef.current.active=false; onCanvasPointerMove._lastDist=null; onCanvasPointerMove._lastMid=null; shapeResizingRef.current = { kind:null,id:null,handle:null,start:null,startLayer:null }; };
  const onCanvasPointerDown = (e) => { if (e.pointerType==="touch") pointersRef.current.set(e.pointerId,{x:e.clientX,y:e.clientY}); const panKey = e.button===1 || e.buttons===4 || e.getModifierState?.("Space"); if (panKey){ e.preventDefault(); panDragRef.current={ active:true, startX:e.clientX, startY:e.clientY, origX:num(bgPan.x), origY:num(bgPan.y) }; } };
  ...
  /* ========== FIXED: float-precise zoom correction ========== */
  const applyZoomAt = (screenX, screenY, factor) => {
    const f = num(factor, 1);
    setBgSize((s) => {
      const curScale = num(s.scale, 1);
      const nextScale = Math.min(5, Math.max(0.2, curScale * f));

      const { left, top, width: cw, height: ch } = getCanvasRect(canvasRef.current);
      const dispW = num(s.baseW || DEFAULT_WORLD.baseW) * curScale;
      const dispH = num(s.baseH || DEFAULT_WORLD.baseH) * curScale;

      const beforeDx = (cw - dispW) / 2 + num(bgPan.x);
      const beforeDy = (ch - dispH) / 2 + num(bgPan.y);
  ...
  const onCanvasWheel = (e) => { if (!(e.shiftKey || e.ctrlKey)) return; e.preventDefault(); const step = e.shiftKey && e.ctrlKey ? 0.15 : 0.1; const factor = 1 + (e.deltaY < 0 ? 1 : -1) * step; applyZoomAt(e.clientX, e.clientY, factor); };
  const nudgeZoom = (m) => { const { left, top, width, height } = getCanvasRect(canvasRef.current); applyZoomAt(left + width / 2, top + height / 2, m); };

  const clearAll = () => snapshotState([], bgUrl, bgSize, [], [], []);
  const removeItem = (id) => snapshotItems(items.filter((it) => it.id !== id));
  const clearBackground = () => { const reset = { baseW: DEFAULT_WORLD.baseW, baseH: DEFAULT_WORLD.baseH, naturalW: 0, naturalH: 0, scale: 1 }; if (lastBgUrlRef.current) { try { URL.revokeObjectURL(lastBgUrlRef.current); } catch {} lastBgUrlRef.current = null; } snapshotState(items, null, reset, walls, windows, floors); };
  ...
  async function saveCompositionImage() {
    try {
      ...
      const outW = Math.max(1, round(dispW, 1)), outH = Math.max(1, round(dispH, 1));
      ...
      if (bgUrl && dispW > 0 && dispH > 0) { await new Promise((resolve) => { const img = new Image(); img.onload = () => { ctx.drawImage(img, 0, 0, outW, outH); resolve(); }; img.onerror = resolve; img.src = bgUrl; }); }
      const drawRects = (list, color) => { ctx.save(); ctx.globalAlpha = 0.35; ctx.fillStyle = color; ctx.strokeStyle = color; for (const r of list) { const leftPx = round(num(r.fx) * refW) - bgDx; const topPx = round(num(r.fy) * refH) - bgDy; const wPx = round(num(r.fw) * refW); const hPx = round(num(r.fh) * refH); ctx.fillRect(leftPx, topPx, wPx, hPx); ctx.strokeRect(leftPx, topPx, wPx, hPx); } ctx.restore(); };
      drawRects(floors, "#0a28a0"); drawRects(walls, "#ff4da6"); drawRects(windows, "#00a050");
      for (const it of items) { const node = document.getElementById(it.id); if (!node) continue; const svg = node.querySelector("svg"); if (!svg) continue; const left0 = round(num(it.fx) * refW) - bgDx; const top0 = round(num(it.fy) * refH) - bgDy; const cloneSvg = svg.cloneNode(true); const svgStr = new XMLSerializer().serializeToString(cloneSvg); const blob = new Blob([svgStr], { type: "image/svg+xml;charset=utf-8" }); const url = URL.createObjectURL(blob); const sizePx = Math.max(16, Math.min(256, num(it.size || 48)));
        await new Promise((resolve) => { const img = new Image(); img.onload = () => { ctx.drawImage(img, left0, top0, sizePx, sizePx); URL.revokeObjectURL(url); resolve(); }; img.onerror = () => { URL.revokeObjectURL(url); resolve(); }; img.src = url; });
      }
      const a = document.createElement("a"); a.href = canvas.toDataURL("image/png"); a.download = "composition.png"; document.body.appendChild(a); a.click(); a.remove();
    } catch (err) { console.error("saveCompositionImage failed", err); alert("Could not save the composition image."); }
  }

  const undo = () => { if (!canUndo) return; const i = hIndex - 1; setHIndex(i); const s = history[i]; setItems(clone(s.items)); setWalls(clone(s.walls || [])); setWindows(clone(s.windows || [])); setFloors(clone(s.floors || [])); setBgUrl(s.bgUrl); setBgSize({ ...s.bgSize }); };
  const redo = () => { if (!canRedo) return; const i = hIndex + 1; setHIndex(i); const s = history[i]; setItems(clone(s.items)); setWalls(clone(s.walls || [])); setWindows(clone(s.windows || [])); setFloors(clone(s.floors || [])); setBgUrl(s.bgUrl); setBgSize({ ...s.bgSize }); };
  ...
  const { refW, refH } = getRefFrame(); // (exposed if needed)

  const Handles = ({ kind, r }) => {
    if (!selecting) return null;
    const leftPx = Math.round(num(r.fx) * refW), topPx = Math.round(num(r.fy) * refH);
    const wPx = Math.round(num(r.fw) * refW), hPx = Math.round(num(r.fh) * refH);
    const cx = leftPx + wPx / 2, cy = topPx + hPx / 2; const color = colorFor(kind);
    const mk = (pos, st, cursor) => (
      <div key={pos} role="button" aria-label={`Resize ${pos}`} style={{ ...styles.resizeHandle(color), ...st, cursor, pointerEvents: "auto" }} onPointerDown={(e) => onShapeHandlePointerDown(e, kind, r.id, pos)} />
    );
    return (
      <>
    ...
    return (
      <>
        {list.map((r) => {
          ...
          return (
            <React.Fragment key={r.id}>
              <div
                style={{ ...styles.rect(fill, stroke), left: leftPx, top: topPx, width: wPx, height: hPx, pointerEvents: "auto" }}
                onPointerDown={(e) => onShapeBodyPointerDown(e, kind, r.id)}
                onDoubleClick={(e) => { if (isWindow) { e.stopPropagation(); openWindowPrompt(r.id, r.heightCm); } }}
                aria-label={kind}
              />
        ...
        {activeTool === kind && selecting && draft && (() => {
          ...
          const l = Math.round(num(rr.fx) * refW), t = Math.round(num(rr.fy) * refH);
          const w = Math.round(num(rr.fw) * refW), h = Math.round(num(rr.fh) * refH);
          return <div style={{ ...styles.draft(draftFill, stroke), left: l, top: t, width: w, height: h, pointerEvents: "none" }} />;
        })()}
      </>
    );
  };
  ...
  const Items = () => {
    ...
    return (
      <>
        {items.map((it) => {
          const left = Math.round(num(it.fx) * refW), top = Math.round(num(it.fy) * refH);
          ...
          return (
            <React.Fragment key={it.id}>
              <div
                ...
                title={`${it.type}`} role="img" aria-label={it.type} tabIndex={-1}
              >
                {Icon[it.type]?.(48)}
              </div>
              <div
                role="button" aria-label="Resize"
                style={{ position: "absolute", left: left + sizePx - 6, top: top + sizePx - 6, zIndex: 11, width: 16, height: 16, borderRadius: 8, background: "#fff", border: "1px solid rgba(0,0,0,0.3)", boxShadow: "0 1px 2px rgba(0,0,0,0.2)", display: "grid", placeItems: "center", cursor: "nwse-resize", touchAction: "none", pointerEvents: "auto" }}
        ...
        {(() => {
          ...
          return (
            <>
              ...
              >
                {bgUrl && (
                  <img
                    ...
                    alt="Background"
                    draggable={false}
                    style={{ position: "absolute", left: 0, top: 0, width: baseW, height: baseH, background: "#fff" }}
                  />
                )}
              ...
              {!bgUrl && (
                <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center", pointerEvents: "none", color: "#666" }}>
      ...
      {windowPrompt && (
        <form style={styles.note} onSubmit={submitWindowHeight}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <input
              autoFocus inputMode="decimal" placeholder="e.g. 40" aria-label="Height above ground in centimeters"
            ...
            />
            <span>cm</span>
            <button type="submit" style={{ height: 32, padding: "0 12px", borderRadius: 8, border: "1px solid rgba(0,0,0,0.2)", background: "#fff", cursor: "pointer", fontWeight: 700 }}>
              Save
            </button>
          </div>
