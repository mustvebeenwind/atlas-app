...
const Icon = {
  ...
  chair: (s = 48) => (
    <svg width={s} height={s} viewBox="0 0 64 64" role="img" aria-label="chair">
      ...
      <rect x="22" y="28" width="20" height="8" rx="2" {...S} />
...
const styles = {
  ...
  rect: (b, s) => ({ position: "absolute", background: b, border: `2px solid ${s}`, borderRadius: 4, pointerEvents: "auto", cursor: "move" }),
  draft: (b, s) => ({ position: "absolute", background: b, border: `2px dashed ${s}`, borderRadius: 4, pointerEvents: "none" }),
  handle: (color) => ({ position: "absolute", width: 12, height: 12, background: "#fff", border: `2px solid ${color}`, borderRadius: 3, boxShadow: "0 1px 2px rgba(0,0,0,0.15)", zIndex: 3, touchAction: "none" }),
...
export default function ImageCanvasApp() {
  ...
  const handleCanvasClick = (e) => {
    ...
    if (activeTool === 'window') {
      setWindowNote("New window placed — what's its height above the ground? (default size 40×200 cm)");
      clearTimeout(handleCanvasClick._noteTimer);
      handleCanvasClick._noteTimer = setTimeout(() => setWindowNote(null), 6500);
    }
  };
  ...
  async function saveCompositionImage() {
    try {
      ...
      const outW = exportOnlyBgArea ? Math.max(1, round(dispW, 1)) : Math.max(1, Math.floor(canvasRef.current.getBoundingClientRect().width));
      const outH = exportOnlyBgArea ? Math.max(1, round(dispH, 1)) : Math.max(1, Math.floor(canvasRef.current.getBoundingClientRect().height));
      const canvas = document.createElement("canvas"); const dpr = Math.max(1, Math.floor(num(window.devicePixelRatio, 1)));
  ...
  const Handles = ({ kind, r }) => {
    ...
    const mk = (pos, st, cursor) => (
      <div key={pos} role="button" aria-label={`Resize ${pos}`} style={{ ...styles.handle(color), ...st, cursor }} onPointerDown={(e) => onShapeHandlePointerDown(e, kind, r.id, pos)} />
    );
    return (
      <>
        ...
        {list.map((r) => {
          ...
          return (
            <React.Fragment key={r.id}>
              <div style={{ ...styles.rect(fill, stroke), left: leftPx, top: topPx, width: wPx, height: hPx }} onPointerDown={(e) => onShapeBodyPointerDown(e, kind, r.id)} aria-label={kind} />
              <Handles kind={kind} r={r} />
            </React.Fragment>
          );
        })}
        {activeTool === kind && selecting && draft && (() => { const rr = normRect(draft.start.fx, draft.start.fy, draft.end.fx, draft.end.fy); const l = Math.round(num(rr.fx) * refW), t = Math.round(num(rr.fy) * refH); const w = Math.round(num(rr.fw) * refW), h = Math.round(num(rr.fh) * refH); return <div style={{ ...styles.draft(draftFill, stroke), left: l, top: t, width: w, height: h }} />; })()}
      </>
    );
  };
