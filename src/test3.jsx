...
const styles = {
  ...
  // renamed: this is the left toggle “grip” button style (must be an object)
  toggleHandle: {
    position: "absolute",
    top: "50%",
    left: 0,
  ...
  // renamed: the per-rectangle resize handle factory
  resizeHandle: (color) => ({
    position: "absolute",
    width: 12,
    height: 12,
...
export default function ImageCanvasApp() {
  ...
  const handleCanvasClick = (e) => {
    ...
    if (activeTool === "window") {
      setWindowNote(
        "New window placed — what's its height above the ground? (default size 40×200 cm) — click to dismiss"
      );
    }
  };
  ...
  const onCanvasPointerMove = (e) => {
    ...
    if (rsz.id) {
      ...
      const size = Math.max(
        16,
        Math.min(256, Math.round(num(rsz.startSize || 48) + d))
      );
  ...
  useEffect(() => {
    try {
      const T = [];
      const push = (n, c) => {
        if (!c) console.warn("Runtime test failed:", n);
        T.push({ n, pass: !!c });
      };
      push(
        "fitWithin square",
        JSON.stringify(fitWithin(1000, 1000, 500, 1000)) ===
          JSON.stringify({ w: 500, h: 500 })
      );
      ...
      push(
        ...
        r.fx === 0.1 &&
          r.fy === 0.2 &&
          Math.abs(r.fw - 0.7) < 1e-9 &&
          Math.abs(r.fh - 0.7) < 1e-9
      );
      push(
        "fitWithin NaN",
        JSON.stringify(fitWithin(NaN, 100, 500, 1000)) ===
      ...
      push("clone undefined", threw === false);
      // extra tests
      push(
        "fitWithin Infinity",
        JSON.stringify(fitWithin(Infinity, 100, 500, 1000)) ===
          JSON.stringify({ w: 0, h: 0 })
      );
      const r2 = normRect(-0.5, 2, 0.2, -1);
      push(
        "normRect clamps",
        r2.fx === 0 && r2.fy === 0 && r2.fw <= 1 && r2.fh <= 1
      );
      // merge: null/number/array inputs handled
      const m1 = merge({ a: 1 }, undefined);
      const m2 = merge(null, { b: 2 });
      const m3 = merge(123, { c: 3 });
      const m4 = merge({ a: 1 }, [1, 2, 3]);
      push("merge undefined", m1.a === 1 && Object.keys(m1).length === 1);
      push("merge null", m2.b === 2 && Object.keys(m2).length === 1);
      push("merge number", m3.c === 3 && Object.keys(m3).length === 1);
      push(
        "merge array rhs ignored keys",
        Object.keys(m4).length === 1 && m4.a === 1
      );
    } catch {}
  }, []);
      ...
      {items.map((it) => {
        ...
        return (
          <React.Fragment key={it.id}>
            ...
            >
              <span
                style={{ fontSize: 10, lineHeight: 1, userSelect: "none", pointerEvents: "none" }}
              >
                ↘︎
              </span>
            </div>
