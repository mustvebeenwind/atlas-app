...
const styles = {
  ...
  // smaller 8×8 handles (kept style; not rendered now)
  resizeHandle: (color) => ({
    position: "absolute",
    width: 8,
    height: 8,
...
export default function ImageCanvasApp() {
  ...
  // shape move/resize refs (kept; handles not rendered)
  ...
  // *** UPDATED: getRel uses the background's displayed rect (world frame)
  const getRel = (x, y) => {
    const { dx, dy, left, top, dispW, dispH } = getBgFrame();
    return {
      fx: clamp01((x - (left + dx)) / Math.max(1, dispW)),
      fy: clamp01((y - (top + dy)) / Math.max(1, dispH)),
      dispW,
      dispH,
    };
  };
  ...
  const onShapeHandlePointerDown = (e, k, id, handle) => {
    // Handles not rendered; keep for future use
    if (!selecting) return;
    e.stopPropagation();
    const r = getLayer(k).find((x) => x.id === id);
    if (!r) return;
  ...
  const onCanvasPointerMove = (e) => {
    // shape resize (not active since handles hidden)
  ...
  async function saveCompositionImage() {
    // (left intact — if you want it adapted to the world wrapper, say the word)
  }
  ...
  // ---------- components (updated to world frame) ----------

  // (We keep Handles component defined in your file, but we don't render it anywhere.)

  // UPDATED: RectLayer without handles, world-frame coords
  const RectLayer = ({ kind }) => {
    ...
    const { dispW, dispH } = getBgFrame();

    return (
      <>
        {list.map((r) => {
          const leftPx = Math.round(num(r.fx) * dispW);
          const topPx  = Math.round(num(r.fy) * dispH);
          const wPx    = Math.round(num(r.fw) * dispW);
          const hPx    = Math.round(num(r.fh) * dispH);
        ...
        {activeTool === kind && selecting && draft && (() => {
          ...
          const l = Math.round(num(rr.fx) * dispW);
          const t = Math.round(num(rr.fy) * dispH);
          const w = Math.round(num(rr.fw) * dispW);
          const h = Math.round(num(rr.fh) * dispH);
          return (
            <div
              style={{
  ...
  // UPDATED: Items in world frame, scale with zoom
  const Items = () => {
    const { dispW, dispH } = getBgFrame();
    const zoom = num(bgSize.scale, 1);

    return (
      <>
        {items.map((it) => {
          const left = Math.round(num(it.fx) * dispW);
          const top  = Math.round(num(it.fy) * dispH);

          const base  = Math.max(16, Math.min(256, num(it.size || 48)));
          const sizePx = base * zoom;
          ...
          return (
            <React.Fragment key={it.id}>
              ...
              {/* Keep the small corner resize nub; remove if you don't want resizing */}
              <div
                role="button"
                aria-label="Resize"
                style={{
    ...
    <div style={styles.app} tabIndex={-1} onMouseDown={defocusActive}>
      ...
      >
        {(() => {
          ...
          return (
            // WORLD WRAPPER: same placement/size as the displayed background
            <div
              style={{
                position: "absolute",
                left: dx,
            ...
            >
              {/* Background fills the world (no rounding) */}
              <img
                src={bgUrl}
                alt="Background"
                draggable={false}
              ...
              {/* Overlays rendered in the same world coordinate system */}
              <div style={{ position: "absolute", inset: 0 }}>
                {/* floor → wall → window → items */}
